...
class WPML_Translation_Management {
	...
	const PAGE_SLUG_MANAGEMENT = '/menu/class.main.php';
	...
	function admin_enqueue_scripts() {
		if ( ! defined( 'DOING_AJAX' ) ) {
			...
			if ( filter_input(INPUT_GET, 'page') === WPML_TM_FOLDER . '/menu/class.main.php' ) {
    ...
    function plugin_action_links($links, $file){
        ...
        if($file == $this_plugin) {
            $links[] = '<a href="admin.php?page='.basename(WPML_TM_PATH) . '/menu/class.main.php">' .
	...
	function automatic_service_selection_pages() {
		return is_admin() &&
					 isset($_GET['page']) &&
					 $_GET['page'] == WPML_TM_FOLDER . '/menu/class.main.php' &&
	...
	private function service_authentication_notice() {
		...
		if ( current_user_can( WPML_Manage_Translations_Role::CAPABILITY ) && $this->service_activation_incomplete() ) {
			...
			if( ! $this->is_translation_services_tab() ) {
				...
				$services_base_url           = admin_url( 'admin.php?page=' . WPML_TM_FOLDER . '/menu/class.main.php' );
		...
		if(isset($notification_message)) {
			$args = array(
				...
				'admin_notice'  => true,
				'hide'          => false,
				'limit_to_page' => array( WPML_TM_FOLDER . '/menu/class.main.php' ),
			);
